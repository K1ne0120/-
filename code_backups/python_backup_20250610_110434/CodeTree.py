...
class CodeTree:
    ...
    #     indent_level = 0 # 当前缩进级别（用于处理嵌套结构）
    ...
    #             self._parse_line(line, current_parent)
    #         else:
    #             # 如果堆栈为空，使用root节点
    #             self._parse_line(line, self.root)
    #
    #     return self.root
    ...
    #
    #                     # 添加块节点并压入堆栈
    #                     block_node = Node("block", parent=else_node)
    #                     self.total_nodes += 1
    #                     stack.append(block_node)
    #                     block_stack.append(block_node)
    #             continue
    ...
    #
    #                 # 如果是控制结构节点
    #                 if node and node.name in ['if', 'for', 'while', 'else if']:
    #                     current_control = node
    #
    #                 # 如果有待处理的else节点
    #                 if pending_else:
    #                     # 将else节点挂载到当前控制节点
    #                     pending_else.parent = current_control
    #                     current_control = pending_else
    #                     pending_else = None
    #
    #             # 添加块节点并压入堆栈
    #             block_node = Node("block", parent=current_control or stack[-1])
    #             self.total_nodes += 1
    #             stack.append(block_node)
    #             block_stack.append(block_node)
    #             continue
    #
    #         # 解析普通行
    #         node = self._parse_line(line, stack[-1])
    #
    #     return self.root
    ...
    # def insert_code(self, code_line, parent_node=None):
    #     """在指定位置插入代码行"""
    #     if not self.root:
    ...
    #
    #     self._parse_line(code_line, parent_node)
    #     return parent_node
    ...
    # def _print_subtree(self, root_node, max_depth=3):
    #     """打印子树结构，带深度和广度限制
    #
    #     参数:
    #         root_node: 子树根节点
    ...
    # def _print_subtree(self, node, depth=0, is_last=False, prefix=''):
    #     """改进的树形输出方法"""
    #     if depth == 0:
    #         connector = f"{node.name}"
    #     else:
    #         connector = "└── " if is_last else "├── "
    #         connector = f"{prefix}{connector}{node.name}"
    #
    #     # 添加表达式信息
    ...
    #         child_lines = self._print_subtree(
