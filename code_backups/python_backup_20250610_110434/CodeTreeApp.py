...
class CodeTreeApp(tk.Tk):
    ...
    #         bracket_errors = tree.detect_bracket_errors(code)
    #         self.log_message(bracket_errors)
    ...
    def build_tree(self, code, is_tree1=True):
        ...
        try:
            ...
            #     self.log_message("\n部分构建的树结构:")
            #     tree_display = self._print_subtree(tree.root, max_depth=3)
            #     self.log_message(tree_display)
    ...
    def insert_code_line(self):
        ...
        # def update_indent_preview():
        #     """更新缩进预览，使用与insert_code_line相同的逻辑"""
        #     try:
        #         selection = line_listbox.curselection()
        #         if not selection:
        #             indent_label.config(text="未选择行")
        #             return
        #
        #         line_idx = selection[0]
        ...
        #
        #         # 确定插入位置（在选中行之后）
        #         insert_position = line_idx + 1
        #
        #         # 计算缩进的逻辑（与insert_code_line相同）
        #         if insert_position >= len(lines):  # 插入到末尾
        #             if lines:  # 代码不为空
        #                 prev_line = lines[-1]
        ...
        #                 else:
        #                     # 检查控制结构上下文
        #                     control_keywords = ["if", "else", "for", "while", "switch", "case", "default", "try",
        #                                         "catch"]
        #                     control_context = any(
        ...
        #                 indent_level = 0
        #         else:  # 在代码中间插入
        #             prev_line = lines[line_idx] if line_idx >= 0 else ""
        #             base_indent = len(prev_line) - len(prev_line.lstrip()) if prev_line else 0
        #
        #             # 检查是否在代码块中
        #             in_block = prev_line.rstrip().endswith('{') if prev_line else False
        #
        #             # 确定正确的缩进级别
        #             if code == '}':
        #                 if in_block:
        ...
        #
        #         # 创建预览
        #         preview_code = " " * indent_level + code
        #
        #         # 显示缩进信息
        #         indent_text = f"缩进级别: {indent_level}空格"
        #         if indent_level > 0:
        #             indent_text += f" → '{preview_code}'"
        #         else:
        #             indent_text += f" → {preview_code}"
        #
        #         indent_label.config(text=indent_text)
        #
        #     except Exception as e:
        #         indent_label.config(text=f"错误: {str(e)}")
